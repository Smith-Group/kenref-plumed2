...
autoreconf --force
./configure CXXFLAGS="-stdlib=libc++ -O0 -g -fPIC -Wall -pedantic -std=c++17 -march=skylake-avx512"\
    LDFLAGS="-L/smithlab/opt/llvm/20.1.1/lib/x86_64-unknown-linux-gnu" \
    LIBS="-Wl,--push-state,--no-as-needed -lc++ -lc++abi -lunwind -Wl,--pop-state -lpthread -ldl" \
    --prefix=/smithlab/opt/plumed-dev/2.10-clang20.1.1 \
